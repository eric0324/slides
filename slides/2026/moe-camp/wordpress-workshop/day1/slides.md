...
<!--
時間為佔位，依實際課表調整。
第 02 堂節奏：網域/DNS/SSL 由示範，學員動手做 FTP 安裝 WordPress（核心實作）。
...
<v-click>
 ...
 今天**講師已用 Cloudways 開好主機**，並幫大家建好**子網域與資料庫**。
...
強調：學員今天不用自己註冊 Cloudways，直接用講師開好的主機與子網域做 FTP 安裝，省去等待與綁卡的時間。
-->
...
今天這三個由**示範**操作，等下會用**講師開好的主機與子網域**直接安裝 WordPress。
...
節奏可快，重點是讓學員知道「主機怎麼來」，把時間留給後面的 FTP 實作。
-->
...
<div class="w-fit mx-auto mt-4 text-sm space-y-1">
  ...
  <div class="flex items-center gap-3 w-[540px] rounded-lg border-2 border-[#0073aa] bg-white px-4 py-2 shadow">
    ...
    <div class="flex-1">
      ...
      <div class="text-xs text-gray-400">你親手用 FTP 裝起來</div>
    </div>
...
2. **第 02 堂**：保全→門牌→房子→內容；你親手用 **FTP** 把 WordPress 啟動了。
